...
/**
 * Lingera Platform Health Checks v3.3
 * 
 * Changes from v3.2:
 * - FIX: ChatGPT "Just a moment..." Cloudflare page has EMPTY body in headless.
 *   Now also checks page TITLE for Cloudflare indicators.
 * - NEW: AWS WAF/CloudFront 403 detection for DeepSeek.
 *   Treats CDN blocks as "alive" (platform exists, just blocks CI IPs).
 * - Tiered detection: Cloudflare/WAF → Brand → Login Wall → Chat Input
 */
...
// ── Cloudflare detection (v3.3: also checks page title for empty-body cases) ──
async function detectCloudflare(page: Page): Promise<boolean> {
  try {
    // v3.3: Check page TITLE first (handles headless empty-body Cloudflare pages)
    const title = await page.title();
    const cfTitlePatterns = [
      /just a moment/i,
    ...
    for (const pattern of cfTitlePatterns) {
      if (pattern.test(title)) {
        console.log(`☁️ Cloudflare detected via page title: "${title}"`);
        return true;
      }
    }

    // Check body text (works when CF page is fully rendered)
    ...
    // Single hit + empty body = likely Cloudflare blocking JS rendering
    if (hits.length >= 1 && bodyText.trim().length < 50) {
      console.log(`☁️ Cloudflare detected: body almost empty + indicator match`);
      return true;
    }
...
// ── AWS WAF / CloudFront block detection (NEW in v3.3) ──
// Some platforms (e.g., DeepSeek) use AWS CloudFront with WAF rules
// that block CI/headless IPs with a 403. This is NOT a failure —
// it means the platform exists and actively serves traffic.
async function detectWAFBlock(page: Page): Promise<boolean> {
  try {
    const title = await page.title();
    const bodyText = await page.locator('body').innerText({ timeout: 3000 });
    ...
    const wafPatterns = [
      ...
      /access denied.*cloudfront/i,
      /aws.*waf/i,
    ];

    const hits = wafPatterns.filter(p => p.test(combined));
    if (hits.length >= 1) {
      console.log(`🛡️ AWS WAF/CloudFront block detected (${hits.length} indicators)`);
      return true;
    }
  } catch { /* ignore */ }
  return false;
  ...
  for (const platform of platforms) {
    test(`${platform.name} - health check`, async ({ page }) => {
      ...
      try {
        ...
        // ── TIER 1b: AWS WAF / CloudFront block ──
        const isWAF = await detectWAFBlock(page);
        if (isWAF) {
          console.log(`🛡️ ${platform.name}: AWS WAF/CloudFront block — platform alive`);
          writeResult({
            platform: platform.name.toLowerCase(),
            status: 'cloudflare',  // treated same as cloudflare (CDN-level block)
            error_message: `AWS WAF/CloudFront block at ${diag.final_url}`,
            error_category: 'cloudflare',
            response_time_ms: Date.now() - startTime,
            diagnostics: diag,
          });
